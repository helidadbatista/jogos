...
export default function FimCaca({ venceu, total, encontradas, pontos, onNovaRodada, onTrocarContexto, onInicio }) {
  return (
    <div className={`tela tela-fim ${venceu ? 'fim-vitoria' : 'fim-derrota'}`}>
      <div className="fim-emoji">{venceu ? '🔎✨' : '🌱'}</div>
      <h1 className="titulo">{venceu ? 'Achou todas!' : 'Quase lá!'}</h1>
      <p className="subtitulo">
        Você encontrou <strong>{encontradas}</strong> de <strong>{total}</strong> palavras
      </p>
